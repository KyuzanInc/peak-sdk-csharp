...
> **COMPLETED.** This migration landed in PR #10 (`c61183e`, consume
> `KyuzanInc.Turnkey.Sdk` from GitHub Packages) and the lock-file
> follow-up PR #11 (`946b909`). Every milestone below is done; the table
...
| M2 | Remove in-repo `packages/turnkey-sdk-csharp/` (src + tests + READMEs); update `peak-sdk-csharp.sln` (drop Turnkey + Tests rows) | ✅ Done |
| M3 | Remove `upstream-snapshots/turnkey-sdk-unity/` and `upstream-snapshots/turnkey-official-src/` (if present); trim `upstream-snapshots/SOURCES.md` rows | ✅ Done |
| M4 | Remove `codex-crypto-reviews/` entirely (README, script, pins, evidence files) | ✅ Done |
| M5 | Remove `docs/security/crypto-port-policy.md` (in-repo crypto policy no longer applicable) | ✅ Done |
| M6 | Add `KyuzanInc.Turnkey.Sdk` `PackageReference` + GitHub Packages source in `nuget.config` + `--locked-mode` lock-file restore | ✅ Done — package reference + nuget.config source + `RestorePackagesWithLockFile=true` + committed `packages.lock.json` files + `--locked-mode` (the lock files were generated by a one-shot CI workflow because the maintainer's gh token lacks `read:packages`). |
| M7 | Rename `global::KyuzanInc.Turnkey.Sdk.*` → `global::Turnkey.*` in **every file under `packages/peak-sdk-csharp/src/**/*.cs` and `packages/peak-sdk-csharp/tests/**/*.cs`** | ✅ Done |
| M8 | Update **both** CI workflows (`csharp-ci.yml` AND `csharp-publish.yml`) for GitHub Packages auth + locked restore + drop dead `codex-review-evidence-check` job | ✅ Done |
| M9 | Update docs / config: `README.md`, `CLAUDE.md`, `docs/architecture.md`, `docs/development.md`, `docs/sync-rules.md`, `docs/security/storage-threat-model.md`, `.github/CODEOWNERS`, `Directory.Packages.props` (remove BouncyCastle pin), `packages/peak-sdk-csharp/README.md` and `README.public.md` | ✅ Done |
| M10 | Add a wire-format smoke test exercising every external method peak-sdk-csharp consumes + Peak wrapper serialization | ✅ Done |
| M11 | Build + `--locked-mode` restore + test green on all three runners; commit; push | ✅ Done — PR #10 merged with plain restore; follow-up branch `chore/m11-lockfiles` committed the CI-generated `packages.lock.json` files and re-enabled `--locked-mode` across all three runners. |
...
| MD8 | Lock-file restore: enable `<RestorePackagesWithLockFile>true</RestorePackagesWithLockFile>` on **both** `packages/peak-sdk-csharp/src/peak-sdk-csharp.csproj` AND `packages/peak-sdk-csharp/tests/peak-sdk-csharp.Tests.csproj`. Commit both `packages.lock.json` files. CI restores with `--locked-mode`. **Generation note:** the lock files were generated by a one-shot CI workflow (`bootstrap-lockfiles.yml`, since removed) because the maintainer's local gh token cannot pull KyuzanInc.Turnkey.Sdk; the workflow uploaded them as an artifact for the maintainer to commit. | One project under lock + one without would split the truth between them. |
...
```
packages/peak-sdk-csharp/src/packages.lock.json     generated by a one-shot CI workflow (bootstrap-lockfiles.yml, since removed) and committed on the chore/m11-lockfiles follow-up branch
packages/peak-sdk-csharp/tests/packages.lock.json   same as above
