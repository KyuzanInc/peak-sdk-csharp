...
| W3  | PR 3: Godot + console smoke examples + Unity reference example | 🟡 Unity portion delivered (Console + Godot still deferred) | `examples/peak-sdk-unity-reference/` + IL2CPP smoke log; design `docs/superpowers/specs/2026-06-09-unity-reference-example-otp-login-design.md`. (W3 had been deferred to v0.1.0-alpha.1, which shipped with .2 without it — the Unity third lands here.) Consumes the SDK via **GitHub Packages** (`github-kyuzan`), not a local file feed — internal-testing default switched per spec r4 (the original local-feed/`prepare-feed.sh` plumbing was removed). | #24 | — |
